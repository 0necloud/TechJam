...
flowchart LR
    ...
    TX --> Stage --> Container["Restricted disposable container"]
...
5. Show `ark-gateway` in the evidence panel. The startup preflight has already
   proved that a Runtime peer cannot contact a public destination directly.
...
In the default supported POC and host-deployment paths, an internal Docker
network and Responses-only gateway enforce Ark-only egress and keep the real
Ark key out of the Runtime. `current-bridge` remains an explicit compatibility
mode for npm/GitHub tasks and restores those network and credential risks.
Ordinary containers are still not hostile multi-tenant isolation, the gateway
trusts Volcengine Ark with model inputs, and the JSON store is single-process.
Profiles unable to launch the Runtime or required gateway fail closed.
